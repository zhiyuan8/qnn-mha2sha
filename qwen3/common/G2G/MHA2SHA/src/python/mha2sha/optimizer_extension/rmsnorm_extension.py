...
from mha2sha.utils.onnx import (
    ...
    get_initializer_value_from_raw_data,
)
...
RMSNORM_PARAM_ENCODING_KEYS = ["out_mul_weight"]
...
def create_branch_rmsnorm_mapping_dict(
    ...
    return {
        ...
        f"{prefix}_out_mul": create_activation_node_mapping_dict(
            input_1_name=mha_rmsnorm_attn_node.out_mul.input[0],
            input_2_name=None,  # Don't include weight as activation
            output_name=mha_rmsnorm_attn_node.out_mul.output[0],
        ),
        # Parameter mappings
        f"{prefix}_out_mul_weight": NodeMappingDict(
            mha_mapping_name_list=["mha_param_name"],
            sha_mapping_name_list=["sha_param_name"],
            mapping_name_dict={
                "mha_param_name": mha_rmsnorm_attn_node.out_mul.input[1],
                "sha_param_name": None
            }
        ),
    }
...
def update_rmsnorm_sha_encoding_name_to_rmsnorm_encoding_mapping_dict(
    ...
    for branch_prefix, rmsnodes in zip(RMSNORM_BRANCH_PREFIX, [query_pattern_dict, key_pattern_dict]):
        ...
        # Handle parameter encodings  
        for node_name in RMSNORM_PARAM_ENCODING_KEYS:
            if node_name == "out_mul_weight":
                node_mapping_dict = rmsnorm_encoding_mapping_dict[branch_prefix+"_"+node_name]
                sha_node_list = getattr(rmsnodes, "out_mul")
                
                if "sha_param_name" in node_mapping_dict.sha_mapping_name_list:
                    # RMS Norm sha has input 0 = weight (parameter)
                    node_mapping_dict.mapping_name_dict["sha_param_name"] = [node.input[0] for node in sha_node_list]
...
class RmsnormExtension:
    ...
    def capture_rmsnorm(self, d_qkv: dict, qk_matmul_node: NodeProto):
        ...
        for dict_key in ["query", "key"]:
            ...
            try:
                ...
                mha_pattern_dict["pow_val"] = get_initializer_value_from_raw_data(
                    2,
                    dtype=np.int32,
                    dims=[1]
                )

                # 3. Find ReduceMean operation after Pow
                ...
                # 4. Find Add operation after ReduceMean
                ...
                mha_pattern_dict["add_val"] = get_initializer_value_from_raw_data(
                    value=1e-6,
                    dtype=np.float32,
                    dims=[1]
                )

                # 5. Find Sqrt operation after Add
                ...
                # 6. Find Div operation after Sqrt
                ...
                mha_pattern_dict["div_val"] = get_initializer_value_from_raw_data(
                    value=1.0,
                    dtype=np.float32,
                    dims=[1]
                )

                # 7. Find Mul operation after Div
                ...
                # Store the multiplication weight
                ...
                # Store the complete RMSNorm pattern following: pow -> reducemean -> add -> sqrt -> div -> mul
    ...
    def create_sha_rmsnorm(
        ...
        rmsnorm_div = self.mha2sha_optim._op_factory.get_div_op_nodes(
            second_transpose,
            rmsnorm_sqrt
        )
        self.mha2sha_optim.model.graph.node.append(rmsnorm_div)

        out_mul_weight = qk_pattern_dict["out_mul_weight"] # this cannot be empty!
        if self.mha2sha_optim.gqa_model and pattern_dict_key == "key":
            out_mul_weight_for_head_num = out_mul_weight
        else:
            if len(out_mul_weight.shape) == 1:
                if out_mul_weight.shape[0] == head_dim:
                    out_mul_weight_for_head_num = out_mul_weight
                else:
                    start_idx = head_num * head_dim
                    end_idx = start_idx + head_dim
                    out_mul_weight_for_head_num = out_mul_weight[start_idx:end_idx]
            else:
                out_mul_weight_for_head_num = out_mul_weight[
                    :, head_num : head_num + 1, ...
                ]

        # Mul operation: weight * normalized
        rmsnorm_out_mul, rmsnorm_mul_init = (
            self.mha2sha_optim._op_factory.get_mul_op(
                out_mul_weight_for_head_num, rmsnorm_div, propose_op_name=f"{pattern_dict_key}_rmsnorm_mul"
            )
        )
        self.mha2sha_optim.model.graph.node.append(rmsnorm_out_mul)
        self.mha2sha_optim.model.graph.initializer.extend(rmsnorm_mul_init)
