...
class QNNQwen2Attention(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
    ...
    def qnn_init(self):
        ...
        if self.q_proj.bias is not None:
            self.q_proj_conv.bias.data.copy_(self.q_proj.bias).to(self.q_proj.weight.device)
        if self.k_proj.bias is not None:
            self.k_proj_conv.bias.data.copy_(self.k_proj.bias).to(self.k_proj.weight.device)
        if self.v_proj.bias is not None:
            self.v_proj_conv.bias.data.copy_(self.v_proj.bias).to(self.v_proj.weight.device)
        if self.o_proj.bias is not None:
            self.o_proj_conv.bias.data.copy_(self.o_proj.bias).to(self.o_proj.weight.device)
    ...
    def qnn_forward(
        ...
        ## HACK: We note that Qwen2 has no RMSNorm on the q, k states
...
class QNNQwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        self.scaling = self.head_dim**-0.5
        self.self_attn = QNNQwen2Attention(config, layer_idx)

        # Add the MLP part, the same as before
        self.mlp = QNNQwen2MLP(config)
        self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class QNNQwen2(Qwen2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.model.layers = nn.ModuleList(
            [QNNQwen2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.model.norm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.vocab_size = config.vocab_size
